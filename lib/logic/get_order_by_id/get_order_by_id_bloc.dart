...
class GetOrderByIdBloc extends Cubit<GetOrderByIdState> {
  GetOrderByIdBloc({required this.repoImp}) : super(const GetOrderByIdState.initial());

  static GetOrderByIdBloc of(BuildContext context) => BlocProvider.of<GetOrderByIdBloc>(context);
...
OrderList model=OrderList();
  get({required int id}) async {
    try {
      ...
      result.fold(
        (l) {
          debugPrint('================= GetOrderById Bloc : ${KFailure.toError(l)}');
      ...
      debugPrint('================= GetOrderById Bloc (Catch): ${e.toString()} ');
      emit(const GetOrderByIdState.error(error: KFailure.someThingWrongPleaseTryAgain()));
    }
  }
}
